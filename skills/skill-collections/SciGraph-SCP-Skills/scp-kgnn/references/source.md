# SCP-KGNN (SciGraph) — Page Content (captured 2026-04-09)

## Summary
KGNN (Knowledge Graph Neural Network) is a knowledge graph dataset and model framework specifically constructed for drug-drug interaction (DDI) prediction tasks. The dataset aims to predict interaction values between drug pairs by combining a parsed DDI matrix with a knowledge graph extracted from biomedical databases, utilizing graph neural network technology.

## KGNN SCP Server
KGNN (Knowledge Graph Neural Network) is a knowledge graph dataset and model framework specifically constructed for drug-drug interaction (DDI) prediction tasks. The core purpose of this dataset is to predict the interaction values between drug pairs by combining the parsed DDI matrix with the knowledge graph extracted from biomedical databases, utilizing graph neural network technology.
...
## Usage
Taking **KGNN (Knowledge Graph Neural Network)** as an example:
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "KGNN"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 KGNN 实验的完整工作流
    result = await client.session.call_tool(
...
The contributors of this knowledge graph data is Hunan University. Please cite the original paper when using this data:
