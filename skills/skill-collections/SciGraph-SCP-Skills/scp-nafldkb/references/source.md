...
- URL: http://scigraph.openkg.cn/scp-nafldkb/
...
**NAFLDkb** is a specialized knowledge base and platform for drug development against non-alcoholic fatty liver disease (NAFLD). It integrates multi-perspective information from research articles and public databases, presenting associations between drugs, targets, clinical trials, and therapeutic strategies as integrated knowledge graphs.
...
Demonstrates calls using `kg_name: "NAFLDkb"`:

- `get_kg_statistics` with `{"kg_name": "NAFLDkb"}` (or omit `kg_name` for all)
- `get_experiment_workflow` with `{"experiment_id": "experiment_1"}`
- `query_cypher` with `{"cypher": "MATCH (e:Experiment:NAFLDkb) RETURN e.id as experiment_id", "kg_name": "NAFLDkb", "limit": 5}`
- `get_entity_details` with `{"entity_identifier": "experiment_1", "kg_name": "NAFLDkb"}`

#### Aknowledgement & Reference

Contributors: Tongji University and Shanghai Institute of Nutrition and Health, Chinese Academy of Sciences.
