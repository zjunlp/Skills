# SCP-Healx (SciGraph) — Page Content (captured 2026-04-09)

## Summary
Healx KG is a multi-source integrated biomedical knowledge graph for rare disease drug repositioning, developed by Healx Ltd. It aims to generate interpretable therapeutic hypotheses through symbolic reasoning and automated filtering mechanisms. The graph integrates public, commercial, and internally curated data, covering core entities such as compounds, diseases, genes, pathways, and their pharmacological, genetic, and clinical relationships.

## Healx SCP Server
Healx KG is a multi-source integrated biomedical knowledge graph for rare disease drug repositioning, developed by Healx Ltd. It aims to generate interpretable therapeutic hypotheses through symbolic reasoning and automated filtering mechanisms. The graph integrates public, commercial, and internally curated data, covering core entities such as compounds, diseases, genes, pathways, and their pharmacological, genetic, and clinical relationships. It has been experimentally validated in fragile X syndrome, cystic fibrosis, and Parkinson’s disease.
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "Healx"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 Healx 实验的完整工作流
    result = await client.session.call_tool(
...
The contributors of this knowledge graph data is Healx Ltd. Please cite the original paper when using this data:
