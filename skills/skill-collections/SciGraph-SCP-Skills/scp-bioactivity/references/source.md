...
- URL: http://scigraph.openkg.cn/scp-bioactivity/
...
**SCP-BioActivity** (Biology)

The page describes **TarIKGC** as the underlying approach: a semantics-enhanced knowledge graph for target identification in small-molecule drug discovery, demonstrated on CDK2 inhibitor development. It integrates structured knowledge from public biomedical databases with semantic relationships extracted from scientific literature to support knowledge graph completion (KGC)-based target prediction.
...
Demonstrates calls using `kg_name: "BioActivity"`:

- `get_kg_statistics` with `{"kg_name": "BioActivity"}` (or omit `kg_name` for all)
- `get_experiment_workflow` with `{"experiment_id": "experiment_1"}`
- `query_cypher` with `{"cypher": "MATCH (e:Experiment:BioActivity) RETURN e.id as experiment_id", "kg_name": "BioActivity", "limit": 5}`
- `get_entity_details` with `{"entity_identifier": "experiment_1", "kg_name": "BioActivity"}`

#### Aknowledgement & Reference

Contributor: Sun Yat-sen University.
