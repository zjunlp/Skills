# SCP-IDP (SciGraph) — Page Content (captured 2026-04-09)

## Summary
IDP-KG (Intrinsically Disordered Protein Knowledge Graph) is a biomedical knowledge graph designed to integrate and represent information about intrinsically disordered proteins (IDPs). The graph aggregates data from multiple specialized protein disorder databases and models them as a semantic knowledge graph using RDF and Bioschemas vocabulary.

## IDP SCP Server
IDP-KG (Intrinsically Disordered Protein Knowledge Graph) is a biomedical knowledge graph designed to integrate and represent information about intrinsically disordered proteins (IDPs). The graph aggregates data from multiple specialized protein disorder databases and models them as a semantic knowledge graph using RDF and Bioschemas vocabulary. It enables structured querying and analysis of protein disorder annotations, protein ensembles, sequence regions, and related publications.
...
## Usage
Taking **IDP(Intrinsically Disordered Protein Knowledge Graph)** as an example:
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "IDP"}  # 不指定 kg_name，返回所有图谱统计
    )
...
The contributors of this knowledge graph data is Heriot-Watt University. Please cite the original paper when using this data:

Gray, A., Papadopoulos, P., Asif, I., Mieti, I., & András Hatos. (2022). Creating and exploiting the intrinsically disordered protein knowledge graph (idp-kg).
