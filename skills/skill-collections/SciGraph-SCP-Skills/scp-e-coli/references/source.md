# SCP-E-Coli (SciGraph) — Page Content (captured 2026-04-09)

## Summary
KIDS constructs an inconsistency-free knowledge graph supporting multiple triple types and knowledge graph completion. The KIDS framework is applied to *Eschericia coli* antibiotic resistance, proposing an integrated approach for automated knowledge representation and discovery, demonstrating evidence-driven decisions that automate knowledge discovery with high confidence and accelerated pace.
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "E-Coli"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 E-Coli 实验的完整工作流
    result = await client.session.call_tool(
...
The contributors of this knowledge graph data is University of California. Please cite the original paper when using this data:
