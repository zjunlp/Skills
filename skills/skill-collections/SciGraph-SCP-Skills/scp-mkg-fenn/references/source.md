# SCP-MKG-FENN (SciGraph) — Page Content (captured 2026-04-09)

## Summary
MKG-FENN is a multimodal knowledge graph fusion system for accurate drug-drug interaction (DDI) prediction. It integrates structured and semi-structured knowledge from multiple authoritative data sources including drug information, protein targets, biological pathways, molecular properties, and clinical interaction data.
...
## Usage
Taking **MKG-FENN(Multimodal Knowledge Graph Fused End-to-end Neural Network)** as an example:
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "MKG-FENN"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 MKG-FENN 实验的完整工作流
    result = await client.session.call_tool(
    ...
    ## 示例3：使用 Cypher 查询 MKG-FENN 相关信息
    result = await client.session.call_tool(
        "query_cypher",
        arguments={
...
The contributors of this knowledge graph data are Chongqing University of Posts and Telecommunications and Southwest University. Please cite the original paper when using this data:
