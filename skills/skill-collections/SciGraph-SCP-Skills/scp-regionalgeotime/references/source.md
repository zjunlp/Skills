# SCP-RegionalGeoTime (SciGraph) — Page Content (captured 2026-04-09)

## Summary
The Regional Geologic Time Standards Knowledge Graph is a machine-readable knowledge graph representing geologic time intervals (Epoch and Age levels) from 17 regional standards worldwide. It integrates these regional standards with the international geologic time scale to form a comprehensive deep-time knowledge base, addressing heterogeneity of geologic time terminology across databases and publications.

## RegionalGeoTime SCP Server
The Regional Geologic Time Standards Knowledge Graph is a machine-readable knowledge graph that systematically represents geologic time intervals (Epoch and Age levels) from 17 regional standards worldwide. It integrates these regional standards with the international geologic time scale to create a comprehensive deep-time knowledge base, addressing the heterogeneity of geologic time terminology across different databases and publications. The knowledge graph enables automated data integration, semantic interoperability, and data-driven discovery in Earth science research, particularly for studies of the co-evolution of the geosphere and biosphere.
...
## Usage
Taking **RegionalGeoTime(Regional Geologic Time Standards Knowledge Graph)** as an example:
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "RegionalGeoTime"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 RegionalGeoTime 实验的完整工作流
    result = await client.session.call_tool(
    ...
    ## 示例3：使用 Cypher 查询 RegionalGeoTime 相关信息
    result = await client.session.call_tool(
        "query_cypher",
        arguments={
...
The contributors of this knowledge graph data is University of Idaho. Please cite the original paper when using this data:
