# SCP-iKraph (SciGraph) — Page Content (captured 2026-04-09)

## Summary
iKraph is a comprehensive, large-scale biomedical knowledge graph designed to support AI-powered and data-driven biomedical research. It integrates structured relationships extracted from PubMed abstracts and external biomedical databases (e.g., Hetionet), covering entities such as genes, drugs, chemicals, and diseases.
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "iKraph"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 iKraph 实验的完整工作流
    result = await client.session.call_tool(
...
The contributors of this knowledge graph data is Florida State University. Please cite the original paper when using this data:
