...
- URL: http://scigraph.openkg.cn/scp-beacon/
...
**SCP-BEACON** (Chemistry)

**BEACON (Bridging Chemical Structure and Conceptual Knowledge)** is a bioinformatics framework and dataset (proposed 2024, BMC Biology). It introduces a bidirectional fusion approach for compound–protein interaction (CPI) prediction, addressing the limitation of methods that treat chemical structures and biological background knowledge in isolation.
...
Demonstrates calls using `kg_name: "BEACON"`:

- `get_kg_statistics` with `{"kg_name": "BEACON"}` (or omit `kg_name` for all)
- `get_experiment_workflow` with `{"experiment_id": "experiment_1"}`
- `query_cypher` with `{"cypher": "MATCH (e:Experiment:BEACON) RETURN e.id as experiment_id", "kg_name": "BEACON", "limit": 5}`
- `get_entity_details` with `{"entity_identifier": "experiment_1", "kg_name": "BEACON"}`

#### Aknowledgement & Reference

Contributors: Hunan University and Xiangtan University.
