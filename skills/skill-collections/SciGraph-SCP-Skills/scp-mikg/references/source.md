...
- URL: http://scigraph.openkg.cn/scp-mikg/
...
**MiKG** is a biomedical knowledge graph for studying the “microbiota–gut–brain axis” (MGB Axis). By integrating regulatory relationships among gut microbiota, neurotransmitters, metabolites, and mental disorders, it transforms scattered literature findings into RDF structures for machine reasoning, aiming to reveal how the gut microenvironment influences host emotional and cognitive functions.
...
Demonstrates calls using `kg_name: "MiKG"`:

- `get_kg_statistics` with `{"kg_name": "MiKG"}` (or omit `kg_name` for all)
- `get_experiment_workflow` with `{"experiment_id": "experiment_1"}`
- `query_cypher` with `{"cypher": "MATCH (e:Experiment:MiKG) RETURN e.id as experiment_id", "kg_name": "MiKG", "limit": 5}`
- `get_entity_details` with `{"entity_identifier": "experiment_1", "kg_name": "MiKG"}`

#### Aknowledgement & Reference

Contributor: Vrije Universiteit Amsterdam.
