...
- URL: http://scigraph.openkg.cn/scp-ddkg/
...
**DDKG** (Attention-based Knowledge Graph Representation Learning for Predicting Drug-drug Interactions) is a biomedical knowledge graph framework for predicting drug–drug interactions (DDIs). It integrates chemical structural attributes of drugs (SMILES sequences) with biomedical triples (e.g., drug–disease, drug–protein relationships) to predict potential drug interactions using attention mechanisms and graph neural networks.
...
Demonstrates calls using `kg_name: "DDKG"`:

- `get_kg_statistics` with `{"kg_name": "DDKG"}` (or omit `kg_name` for all)
- `get_experiment_workflow` with `{"experiment_id": "experiment_1"}`
- `query_cypher` with `{"cypher": "MATCH (e:Experiment:DDKG) RETURN e.id as experiment_id", "kg_name": "DDKG", "limit": 5}`
- `get_entity_details` with `{"entity_identifier": "experiment_1", "kg_name": "DDKG"}`

#### Aknowledgement & Reference

Data contributor: Xinjiang Technical Institute of Physics & Chemistry, CAS.
