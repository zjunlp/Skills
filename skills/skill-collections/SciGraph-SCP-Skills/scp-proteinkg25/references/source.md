...
- URL: http://scigraph.openkg.cn/scp-proteinkg25/
...
**SCP-ProteinKG25** (Biology)

**ProteinKG25** is a large-scale knowledge graph integrating Gene Ontology (GO) structure with protein sequences and textual definitions. It aligns GO terms (natural language descriptions) and proteins (amino acid sequences) via:

- Internal GO–GO relations (e.g., `is_a`, `part_of`)
- External Protein–GO annotations

It is designed for knowledge-enhanced pretraining of protein language models such as OntoProtein.
...
Demonstrates calls using `kg_name: "ProteinKG25"`:

- `get_kg_statistics` with `{"kg_name": "ProteinKG25"}` (or omit `kg_name` for all)
- `get_experiment_workflow` with `{"experiment_id": "experiment_1"}`
- `query_cypher` with `{"cypher": "MATCH (e:Experiment:ProteinKG25) RETURN e.id as experiment_id", "kg_name": "ProteinKG25", "limit": 5}`
- `get_entity_details` with `{"entity_identifier": "experiment_1", "kg_name": "ProteinKG25"}`
...
Citation:
- Zhang, N., Bi, Z., Liang, X., Cheng, S., Hong, H., Deng, S., Zhang, Q., Lian, J., & Chen, H. (2022). *OntoProtein: Protein pretraining with gene ontology embedding*. International Conference on Learning Representations (ICLR). https://openreview.net/forum?id=yfe1VMYAXa4
