# SCP-KG-MTL (SciGraph) — Page Content (captured 2026-04-09)
...
## Usage
Taking **KG-MTL(Knowledge Graph Enhanced Multi-Task Learning for Molecular Interaction)** as an example:
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "KG-MTL"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 KG-MTL 实验的完整工作流
    result = await client.session.call_tool(
