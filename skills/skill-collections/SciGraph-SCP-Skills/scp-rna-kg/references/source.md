# SCP-RNA-KG (SciGraph) — Page Content (captured 2026-04-09)

## Summary
RNA-KG is an RNA-centered heterogeneous biomedical knowledge graph. It integrates traditional entity–relationship information and introduces contextual properties to address “disconnection from context” in biomedical data, enabling accurate representation of the specific biological context in which RNA interactions occur.

## RNA-KG SCP Server
RNA-KG is an RNA-centered heterogeneous biomedical knowledge graph (KG). It not only integrates traditional “entity-relationship” information but also introduces contextual properties. It addresses the common issue of “disconnection from context” in biomedical data and is capable of accurately describing the specific biological context in which RNA interactions occur.
...
async def main():
    ...
    result = await client.session.call_tool(
        "get_kg_statistics",
        arguments={"kg_name": "RNA-KG"}  # 不指定 kg_name，返回所有图谱统计
    )
    stats = client.parse_result(result)
    print(stats)

    ## 示例2：查询 RNA-KG 实验的完整工作流
    result = await client.session.call_tool(
...
The contributors of this knowledge graph data is University of Milan. Please cite the original paper when using this data:
